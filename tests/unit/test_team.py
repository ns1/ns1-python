...
@pytest.mark.parametrize('team_id, name, ip_whitelist, permissions, url',
                         [('1', 'test-team',
                             [{'name': 'Test Whitelist', 'values': ['1.1.1.1']}],
                             {'data': {'push_to_datafeeds': True}},
                             'account/teams/1')])
def test_rest_team_update(team_config, team_id, name, ip_whitelist,
        permissions, url):
    z = ns1.rest.team.Team(team_config)
    z._make_request = mock.MagicMock()
    z.update(team_id, name=name, ip_whitelist=ip_whitelist, permissions=permissions)
