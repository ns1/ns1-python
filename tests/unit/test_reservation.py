...
def test_rest_reservation_list(reservation_config):
    ...
    z._make_request.assert_called_once_with(
        "GET", "dhcp/reservation?scopeGroupId=1", callback=None, errback=None
    )


@pytest.mark.parametrize("reservation_id, url", [(1, "dhcp/reservation/1")])
...
@pytest.mark.parametrize(
    "sgroup_id, address_id, mac, options, url",
    [
        (
            1,
            2,
...
def test_rest_reservation_create(
    reservation_config, sgroup_id, address_id, mac, options, url
):
    z = ns1.rest.ipam.Reservations(reservation_config)
    z._make_request = mock.MagicMock()
