...
def test_rest_network_retrieve(network_config, network_id, url):
    ...
    z._make_request.assert_called_once_with(
        "GET", url, callback=None, errback=None, params={"expand": True}
    )
...
@pytest.mark.parametrize(
    "network_name, url", [("test_network", "ipam/network")]
)
def test_rest_network_create(network_config, network_name, url):
    ...
    z._make_request.assert_called_once_with(
        "PUT", url, callback=None, errback=None, body={"name": network_name}
    )


@pytest.mark.parametrize(
    "network_id, network_desc, url",
...
def test_rest_network_update(network_config, network_id, network_desc, url):
    ...
    z._make_request.assert_called_once_with(
        "POST", url, callback=None, errback=None, body={"desc": network_desc}
    )
