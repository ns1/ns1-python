...
@pytest.mark.parametrize('apikey_id, name, ip_whitelist, permissions, url',
                         [('test-apikey_id', 'test-apikey',
                             ['1.1.1.1','2.2.2.2'],
...
def test_rest_apikey_update(apikey_config, apikey_id, name, ip_whitelist,
    ...
    z = ns1.rest.apikey.APIKey(apikey_config)
    z._make_request = mock.MagicMock()
    z.update(apikey_id, name=name, ip_whitelist=ip_whitelist, permissions=permissions)
...
@pytest.mark.parametrize('apikey_id, url', [('test-apikey_id',
    'account/apikeys/test-apikey_id')])
