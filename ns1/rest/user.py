...
from . import resource, permissions
...
class User(resource.BaseResource):
    ...
    def create(self, name, username, email, callback=None, errback=None, **kwargs):
        body = {
            'name': name,
            'username': username,
            'email': email
    ...
    def delete(self, username, callback=None, errback=None):
        return self._make_request('DELETE', '%s/%s' % (self.ROOT, username),
                                  callback=callback,
                                  errback=errback)
    ...
    def retrieve(self, username, callback=None, errback=None):
        return self._make_request('GET', '%s/%s' % (self.ROOT, username),
                                  callback=callback,
                                  errback=errback)
